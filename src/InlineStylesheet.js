...
class InlineStylesheet {
  ...
  static create(base, ...rest) {
    ...
    if (base === null || base === undefined || typeof base === 'string') {
      ...
      if (
        modifier != null &&
        (typeof modifier !== 'object' || Array.isArray(modifier))
      ) {
        throw new Error(
      ...
      return new InlineStylesheet([
        new StaticStylesheet({ _base: base || '', ...modifier }),
      ]);
    }
  ...
  concat(sheetOrModifier) {
    ...
    if (
      sheetOrModifier instanceof StaticStylesheet ||
      sheetOrModifier instanceof TaggedStylesheet
    ) {
      return new InlineStylesheet([...this._sheets, sheetOrModifier]);
    }

    // Convert modifier object to static stylesheet
    if (
      typeof sheetOrModifier === 'object' &&
      !Array.isArray(sheetOrModifier)
    ) {
      return new InlineStylesheet([
        ...this._sheets,
        new StaticStylesheet(sheetOrModifier),
      ]);
    }
